...
import { IMetaPagination, IMetaPurchasePagination, IPaginationQuery } from "../../types/APITypes/BasicAPITypes";
...
export const GameAPI = {
  ...
  fetchPurchaseHistory: async (query: IPaginationQuery = { page: 1, limit: 20 }): Promise<{ data: IPurchaseHistory[]; meta: IMetaPurchasePagination }> => {
    try {
      const res = await axiosAuth.get<{ data: IPurchaseHistory[]; meta: IMetaPurchasePagination }>(`/purchase`, { params: query });
      return res.data;
    } catch (err) {
